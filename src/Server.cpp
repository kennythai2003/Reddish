    ...
    for (int fd = 0; fd <= fd_max; ++fd) {
      if (FD_ISSET(fd, &read_fds)) {
        if (fd == server_fd) {
          ...
          // Data from master (propagated commands)
          char buffer[4096] = {0};
          int bytes_read = read(master_fd, buffer, sizeof(buffer));
          if (bytes_read <= 0) {
            if (bytes_read < 0) std::cerr << "failed to read from master fd=" << master_fd << "\n";
            else std::cout << "Master connection closed: fd=" << master_fd << "\n";
            close(master_fd);
            ...
            std::string& buf = master_leftover;
            size_t pos = 0;
            while (pos < buf.size()) {
          ...
          // ...existing code for client handling...
          char buffer[1024] = {0};
          int bytes_read = read(fd, buffer, sizeof(buffer));
          if (bytes_read <= 0) {
            // ...existing code for client disconnect and cleanup...
            ...
            // ...existing code for per-client buffer, parsing, and command handling...
